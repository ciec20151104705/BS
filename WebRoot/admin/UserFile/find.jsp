...
<script type="text/javascript">
	function topPage() {
		...
		window.location = contextPath
				+ '/UserFile-getAllUtil.action?pageModel.currentPageNo='
...
<body>
	...
	<div class="rightinfo">
		<div class="tools">
			...
			<form
				action="${pageContext.request.contextPath}/UserFile-getAllUtil.action">
				<ul class="toolbar1" style="float: left;margin-top:-10px;">
					...
					<li style="margin-left: 26px;"><select id="field" name="field"
						style="font-size: 20px; background-color: #F9F9F9; margin: 3px 0px 0px 10px;">
							...
							<option <c:if test="${field == 'userLeave.s_0'}">selected</c:if>
								value="userLeave.s_0" /> 请假名称
							</option>
...
<input class="btn btn-inverse" type="button" value="打&nbsp;&nbsp;印" onclick="javascript:window.print();" style="width: 80px; margin-left: 10px;" />
		...
		<form
			action="${pageContext.request.contextPath}/UserFile-deleteManyUtil.action"
					...
					<c:if test="${pageModel.list != 'null'}">
						<c:forEach var="tmp" items="${pageModel.list}">
							<tr>
								...
								<td><img
									src="${pageContext.request.contextPath}/upload/${tmp.s_0 }"
									width="60" height="20" /></td>
								<td>${tmp.s_1 }</td>
								<td>${tmp.s_2 }</td>
								<td>${tmp.userLeave.s_0 }</td>
								<td>${tmp.s_3 }</td>
								<td>${tmp.s_4 }</td>
								<td><a
									...
									&nbsp;&nbsp; <c:if test="${type == 'admin'}">
										<a target="Conframe" class="tablelink"
											href="${pageContext.request.contextPath}/UserFile-selectUtil.action?util.id=${tmp.id }">修改
										</a>
										<a target="Conframe" class="tablelink"
											href="${pageContext.request.contextPath}/UserFile-deleteUtil.action?util.id=${tmp.id }">删除
										</a>
									</c:if> <c:if test="${type != 'admin'}">
										<a target="Conframe" class="tablelink"
											href="${pageContext.request.contextPath}/UserFile-selectUtil.action?util.id=${tmp.id }">查看
										</a>
									</c:if></td>
							</tr>
						</c:forEach>
